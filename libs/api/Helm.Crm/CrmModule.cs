...
{
    public static IServiceCollection AddCrmModule(this IServiceCollection services, IConfiguration config)
    {
        services.AddDbContext<CrmDbContext>(o => o
            .UseNpgsql(config.GetConnectionString("Helm"),
                npgsql => npgsql.MigrationsHistoryTable("__ef_migrations", "crm")));

        services.AddScoped<ICrmService, CrmService>();
        services.AddHealthChecks().AddCheck<CrmHealthCheck>("crm");
        services.AddHostedService<OutboxRelay<CrmDbContext>>();

        HelmModuleRegistry.Register("Crm", "crm");
...
/// <summary>
/// Cheap liveness check: can we open a raw connection to the database backing the crm schema.
/// Deliberately does not resolve <see cref="CrmDbContext"/> — that requires an
...
/// </summary>
internal class CrmHealthCheck(IConfiguration config) : IHealthCheck
{
